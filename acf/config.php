...
add_action('acf/init', 'seoleadertheme_acf_init');
/**
 ...
 * @return void
 */
function seoleadertheme_acf_init()
{
  add_filter(
    ...
    function ($categories, $post) {
      return array_merge($categories, [
        [
          'slug' => 'seoleadertheme',
          'title' => __('seoleadertheme Blocks', 'seoleadertheme'),
        ],
      ]);
    },
    10,
  ...
  if (function_exists('acf_register_block')) {
    ...
    if (is_array($blocks)) {
      foreach ($blocks as $name => $params) {
        $params = array_merge($params, [
          'name' => $name,
          'mode' => 'edit',
          'render_callback' => 'seoleadertheme_block_render_callback',
        ]);
    ...
    function seoleadertheme_block_render_callback($block)
    {
...
add_filter('acf/settings/save_json', 'seoleadertheme_acf_json_save_point');
/**
 ...
 */
function seoleadertheme_acf_json_save_point($path)
{
  return get_stylesheet_directory() . '/acf/local-json';
}

add_filter('acf/settings/load_json', 'seoleadertheme_acf_json_load_point');
/**
 ...
 */
function seoleadertheme_acf_json_load_point($paths)
{
  unset($paths[0]);
